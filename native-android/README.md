...
> Lives alongside the root [ChoMU](../README.md) app in this repo as a second,
> native (non-Capacitor) take on the same idea — text/image → 3D via NVIDIA
> TRELLIS. ChoMU is React + Capacitor and calls TRELLIS directly; this one is
> Kotlin + Jetpack Compose + Filament and adds fal.ai/Pollinations as
> alternate 3D providers since NVIDIA's free endpoint currently only accepts
> its own sample images for image-to-3D. Fixes applied on top of the
> uploaded source in this session:
> - `ModelViewer.kt` had no light or indirect-light setup, which renders
>   Filament scenes as a black silhouette — added a main directional light
>   and a soft indirect light (see the doc comment on `ModelViewer`).
> - fal.ai's TRELLIS API genuinely supports a `texture_size` field, but only
>   512/1024/2048 — wired it into Settings/FalRepository as a real control
>   rather than claiming a 4K/8K that fal.ai's servers don't produce.
> - Verified `./gradlew assembleDebug` builds clean (arm64-v8a/armeabi-v7a/
>   x86_64/universal APKs) against compileSdk 35.
