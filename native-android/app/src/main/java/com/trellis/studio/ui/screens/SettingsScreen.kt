...
class SettingsViewModel(application: Application) : AndroidViewModel(application) {
    ...
    val falTextureSize = settings.falTextureSize
    ...
    fun setFalTextureSize(size: Int) = settings.setFalTextureSize(size)
}

@Composable
fun SettingsScreen(viewModel: SettingsViewModel = viewModel()) {
    val provider by viewModel.provider.collectAsState()
    val falTextureSize by viewModel.falTextureSize.collectAsState()
    ...
    Box(Modifier.fillMaxSize()) {
        Column(
            ...
            if (provider == Model3DProvider.FAL_TRELLIS) {
                Card(colors = CardDefaults.cardColors(containerColor = MaterialTheme.colorScheme.surface)) {
                    Column(Modifier.padding(16.dp), verticalArrangement = Arrangement.spacedBy(8.dp)) {
                        Text("Texture resolution", style = MaterialTheme.typography.titleMedium)
                        Text(
                            "fal.ai's TRELLIS model's own API caps texture_size at 2048px — there is " +
                                "no 4K or 8K option because fal.ai's servers don't generate one; " +
                                "picking a higher value here wouldn't do anything real.",
                            style = MaterialTheme.typography.bodySmall,
                            color = MaterialTheme.colorScheme.onSurfaceVariant
                        )
                        Row(
                            ...
                            SettingsRepository.FAL_TEXTURE_SIZES.forEach { size ->
                                val selected = size == falTextureSize
                                Row(
                                    modifier = Modifier
                                        .weight(1f)
                                        .selectable(
                                            selected = selected,
                                            onClick = { viewModel.setFalTextureSize(size) },
                                            role = Role.RadioButton
                                        )
                                        .padding(vertical = 8.dp),
                                    ...
                                    Text("${size}px", style = MaterialTheme.typography.bodyMedium)
                                }
                            }
                        }
                    }
